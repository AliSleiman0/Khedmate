...
class _NavigationPageState extends ConsumerState<NavigationPage> {
  ...
  Future<void> _fetchProviderLocation() async {
    try {
      ...
      if (mounted) {
        setState(() => _providerLocation = LatLng(pos.latitude, pos.longitude));
      }
    } catch (_) {}
  }
  ...
  Future<void> _advanceStatus() async {
    ...
    if (job.status == 'InProgress') {
      // Provider must upload after-photos before completing
      if (mounted) context.push('/active-job/${widget.jobId}/after-photos');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return asyncJob.when(
      ...
      data: (job) {
        ...
        return Scaffold(
          appBar:
              AppBar(title: Text('${s.navPageTitle} #${widget.jobId}')),
          body: Stack(
            children: [
              FlutterMap(
                options: MapOptions(
