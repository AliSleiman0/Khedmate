...
class JobRepository {
  ...
  Future<List<JobDetail>> getActiveJobs() async {
    ...
    return items.map((e) => JobDetail.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<JobDetail> advanceJobStatus(String jobId) async {
    final response = await _client.dio.post('/providers/jobs/$jobId/advance');
    // The advance endpoint returns AdvanceJobStatusResultDto, not a full JobDetail.
    // Re-fetch the job so the screen gets the full updated detail.
  ...
  Future<void> sendLocationForJob(String jobId, double latitude, double longitude) async {
  ...
  Future<List<String>> uploadAfterPhotos(String jobId, List<XFile> photos) async {
