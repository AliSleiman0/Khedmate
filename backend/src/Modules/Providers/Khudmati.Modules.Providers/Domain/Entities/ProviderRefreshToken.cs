namespace Khudmati.Modules.Providers.Domain.Entities;

public class ProviderRefreshToken
{
    public Guid Id { get; private set; } = Guid.NewGuid();
    public Guid AccountId { get; private set; }
    public string TokenHash { get; private set; } = string.Empty;
    public DateTime ExpiresAt { get; private set; }
    public DateTime CreatedAt { get; private set; } = DateTime.UtcNow;

    private ProviderRefreshToken() { }

    public static ProviderRefreshToken Create(Guid accountId, string tokenHash, DateTime expiresAt) =>
        new() { AccountId = accountId, TokenHash = tokenHash, ExpiresAt = expiresAt };

    public static ProviderRefreshToken Create(Guid id, Guid accountId, string tokenHash, DateTime expiresAt) =>
        new() { Id = id, AccountId = accountId, TokenHash = tokenHash, ExpiresAt = expiresAt };
}
