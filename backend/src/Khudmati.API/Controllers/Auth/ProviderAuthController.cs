...
public class ProviderAuthController : ControllerBase
{
    ...
    public ProviderAuthController(IMediator mediator)
    {
        _mediator = mediator;
    }

    [HttpPost("register")]
    public async Task<IActionResult> Register([FromBody] RegisterProviderCommand command, CancellationToken ct)
    {
        try
        {
    ...
    [HttpPost("verify-otp")]
    public async Task<IActionResult> VerifyOtp([FromBody] VerifyProviderOtpCommand command, CancellationToken ct)
    {
        try
        {
            var result = await _mediator.Send(command, ct);
    ...
    [HttpPost("login")]
    public async Task<IActionResult> Login([FromBody] LoginProviderCommand command, CancellationToken ct)
    {
        try
        {
            var result = await _mediator.Send(command, ct);
    ...
    [HttpPost("resend-otp")]
    public async Task<IActionResult> ResendOtp([FromBody] ResendProviderOtpCommand command, CancellationToken ct)
    {
        try
        {
            var result = await _mediator.Send(command, ct);
    ...
    [HttpPost("refresh")]
    public async Task<IActionResult> Refresh([FromBody] RefreshProviderTokenCommand command, CancellationToken ct)
    {
        try
        {
            var result = await _mediator.Send(command, ct);
    ...
    public async Task<IActionResult> Logout(CancellationToken ct)
    {
        ...
        try
        {
            var result = await _mediator.Send(new LogoutProviderCommand(accountId), ct);
            return ResultToResponse(result);
        }
        catch (ValidationException ex)
        {
