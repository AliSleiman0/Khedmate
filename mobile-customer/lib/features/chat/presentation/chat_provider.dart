...
class ChatNotifier extends FamilyAsyncNotifier<ChatState, String> {
  ...
  Future<ChatState> build(String arg) async {
    ...
    // Mark existing messages as read immediately
    try {
  ...
  void _subscribeToMessages(String jobId) {
    ref.read(signalRServiceProvider).on('NewChatMessage', (args) {
      ...
      // Mark as read immediately since the screen is open
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      ...
      // The sent message will arrive back via SignalR only if the server sends
      // it to the recipient. Sender must show it immediately without duplication.
      // Since the API returns the saved message, we could add it here, but
      // per spec: sender does NOT receive their own message via SignalR.
      // We reload after send to show the message.
      await _reloadLastPage();
    } catch (e) {
      state = AsyncData(current.copyWith(
          isSending: false, errorMessage: 'فشل في إرسال الرسالة'));
      return;
    }
...
// ---------------------------------------------------------------------------
// Unread count — simple FutureProvider family
// ---------------------------------------------------------------------------
