...
class _TrackingPageState extends ConsumerState<TrackingPage> {
  ...
  Future<void> _ensureSignalR() async {
    try {
      ...
      // SignalR not critical — fallback is pull-to-refresh
    }
  }

  @override
  Widget build(BuildContext context) {
    final asyncTracking = ref.watch(jobTrackingNotifierProvider(widget.jobId));

    // Determine if chat FAB should be shown
...
class _LiveTrackingMapState extends ConsumerState<_LiveTrackingMap>
  ...
  void didUpdateWidget(_LiveTrackingMap oldWidget) {
    ...
    // Animate provider pin movement when location updates
                  ...
                  children: [
                    ...
                    MarkerLayer(
                      markers: [
                        ...
                        // Provider location marker (animated)
                        if (_animatedProviderPosition != null)
                          Marker(
                            ...
                            child: Column(
                              children: [
                                Container(
                                  ...
                                  child: Text(
                                    widget.tracking.providerName.split(' ').first,
                                    style: const TextStyle(
                                      fontFamily: 'Cairo',
                                      fontSize: 11,
                                      color: Colors.white,
