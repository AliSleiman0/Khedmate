    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: asyncState.when(
          ...
          error: (e, _) =>
              _ExpiredState(onBack: () => context.go('/provider/jobs')),
          data: (state) {
            if (state.isExpired && !state.isAccepted) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (context.mounted) {
                  Future.delayed(const Duration(seconds: 2), () {
                    if (context.mounted) context.go('/provider/jobs');
                  });
                }
              });
            }
            ...
            if (state.isAccepted) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (context.mounted) {
                  ...
                  context.go('/provider/navigation/$jobId');
                }
              });
            }
      ...
      child: Column(
        ...
        children: [
          ...
          _SectionCard(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    '${job.district}  •  ${s.distanceKm(job.distanceKm)}',
                    style: const TextStyle(fontFamily: 'Cairo', fontSize: 14),
                  ),
                ),
              ],
          ...
          if (job.beforePhotoUrls.isNotEmpty) ...[
            const SizedBox(height: 12),
            _SectionCard(
              title: s.providerJobBeforePhotos,
              child: SizedBox(
                height: 80,
                child: ListView(
